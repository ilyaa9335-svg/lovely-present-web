...
export default function CheckoutPage() {
  ...
  if (items.length === 0 && !submitted) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <h1 className="font-serif text-2xl text-[var(--color-navy)] mb-4">{t.checkout.emptyCart}</h1>
  ...
  if (submitted) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <h1 className="font-serif text-3xl text-[var(--color-navy)] mb-3">{t.checkout.orderReceived}</h1>
        <p className="text-gray-600 mb-6 max-w-md mx-auto">
          {t.checkout.orderConfirmMessage.replace("{name}", form.name)}
        </p>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <h1 className="font-serif text-3xl font-bold text-[var(--color-navy)] mb-8">{t.checkout.pageTitle}</h1>

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        {/* Left: Form - 3 cols */}
        <form onSubmit={handleSubmit} className="lg:col-span-3 space-y-6">
          <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm space-y-4">
            <h2 className="font-serif text-lg font-bold text-[var(--color-navy)]">{t.checkout.contactDetails}</h2>
          ...
          {/* Delivery method */}
          <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm space-y-4">
            <h2 className="font-serif text-lg font-bold text-[var(--color-navy)]">{t.checkout.deliveryMethod}</h2>
            <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => setDeliveryMethod("pickup")} className={`p-4 rounded-xl border-2 text-left transition-all cursor-pointer ${deliveryMethod === "pickup" ? "border-[var(--color-pink-brand)] bg-[var(--color-pink-light)]" : "border-gray-200 hover:border-gray-300"}`}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-semibold text-[var(--color-navy)]">{t.checkout.pickup}</span>
                </div>
                <p className="text-xs text-gray-500">{t.checkout.pickupAddress}</p>
                <p className="text-sm font-medium text-green-600 mt-1">{t.checkout.free}</p>
              </button>
              <button type="button" onClick={() => setDeliveryMethod("wolt")} className={`p-4 rounded-xl border-2 text-left transition-all cursor-pointer ${deliveryMethod === "wolt" ? "border-[var(--color-pink-brand)] bg-[var(--color-pink-light)]" : "border-gray-200 hover:border-gray-300"}`}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-semibold text-[var(--color-navy)]">{t.checkout.woltDelivery}</span>
                </div>
                <p className="text-xs text-gray-500">{t.checkout.deliveredToDoor}</p>
                <p className="text-sm font-medium text-[var(--color-pink-brand)] mt-1">{t.checkout.woltFee}</p>
              </button>
            </div>

            {deliveryMethod === "wolt" && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t.checkout.deliveryAddress} *</label>
                <input type="text" name="address" required value={form.address} onChange={handleChange} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-base focus:outline-none focus:border-[var(--color-pink-brand)] transition-colors" placeholder={t.checkout.addressPlaceholder} />
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
            <label className="block text-sm font-medium text-gray-700 mb-1">{t.checkout.orderNote}</label>
            <textarea name="note" value={form.note} onChange={handleChange} rows={3} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-base focus:outline-none focus:border-[var(--color-pink-brand)] transition-colors resize-none" placeholder={t.checkout.notePlaceholder} />
          </div>
          ...
          <button type="submit" className="w-full py-4 rounded-full bg-[var(--color-pink-brand)] text-white font-semibold text-lg hover:bg-[#d1177d] transition-all shadow-lg hover:shadow-xl cursor-pointer">
            {t.checkout.placeOrder} — {total.toLocaleString()} Kč
          </button>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm sticky top-24">
            <h2 className="font-serif text-lg font-bold text-[var(--color-navy)] mb-4">{t.checkout.orderSummary}</h2>
              ...
              {items.map(({ product, quantity }) => (
                <div key={product.id} className="flex gap-3 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">{t.checkout.qty} {quantity}</p>
                  </div>
            ...
            <div className="border-t border-gray-100 pt-3 space-y-2 text-sm">
              <div className="flex justify-between text-gray-600"><span>{t.checkout.subtotal}</span><span>{subtotal.toLocaleString()} Kč</span></div>
              <div className="flex justify-between text-gray-600"><span>{t.checkout.delivery}</span><span>{deliveryFee === 0 ? t.checkout.free : `${deliveryFee} Kč`}</span></div>
              <div className="flex justify-between font-bold text-[var(--color-navy)] text-xl pt-2 border-t border-gray-100"><span>{t.checkout.total}</span><span>{total.toLocaleString()} Kč</span></div>
            </div>
          </div>
        </div>
      </div>
